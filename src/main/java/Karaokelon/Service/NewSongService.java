package Karaokelon.Service;

import Karaokelon.Domain.DTO.NewSongDTO;
import Karaokelon.Domain.DTO.TJNewSongDTO;
import Karaokelon.Domain.Entity.MemberEntity;
import Karaokelon.Domain.Entity.MemberRepository;
import Karaokelon.Domain.Entity.NewSongEntity;
import Karaokelon.Domain.Entity.NewSongRepository;
...
//				System.out.println(songArray[i]);
	...
	public ArrayList<String> FindReservationSinger(int mno, ArrayList<NewSongDTO> newChartSong) {
		...
		String[] getReserv = null;

		if(memberEntities.get().getMreserv() != null) {
			getReserv = memberEntities.get().getMreserv().split(",");
		}
				...
				for (int j = 0; j < newChartSong.size(); j++) {
					...
					if(newSong.compareTo(calendar.getTime()) >= 0) {
						if (getReserv[i].contains(newChartSong.get(j).getS_singer())) {
							reserveSinger.add("예약하신 " + getReserv[i] + "님의 신곡이 발매됐어요!");
						}
					}
				}
			}
